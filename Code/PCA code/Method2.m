...
X = studata327(1).knnNmergey;
[row, col] = size(X);
PR = (row - PR_Mapping2(X))/row;
[U,Z,latent,tsquared,explained] = pca(PR*100);
...
disp(Z(:,1:K));
pause;
courseNum = length(studata327(1).info.notmergecourename);
Ordered = [];
coreCourses = {};
for pc = 1:courseNum
    ...
    for i = 1:courseNum
        courseOrder = [courseOrder, studata327(1).info.notmergecourename(sortedIndex(i))];
    end
