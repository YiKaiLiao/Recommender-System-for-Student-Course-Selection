...
function result = fillMissingValue(dataset, merge)
    ...
    for i = 1:size(dataset, 2)
        ...
        t.ppca = (rppca); %rmse of ppca
        t.stu = (rstu); %rmse of knn student
        t.cour = (rcour); %rmse of knn course
        t.ppcaPre = ppcaPred; %prediction result of ppca
        t.stuPre = stuPred; %prediction result of knn student
        t.courPred = courPred; %prediction result of knn course
        t.original = testy; %the original matrix --> for comparison
        t.ppcamin = ppca_min; %the min ppca rmse value
        t.courmin = cour_min; %the min knn course rmse value
        t.stumin = stu_min; %the min knn student rmse value
        result{i} = t;
    end
end
